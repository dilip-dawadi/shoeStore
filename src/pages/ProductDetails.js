...
import { addCarts } from '../statemanagement/slice/cartSlice';
...
const ProductDetails = () => {
  ...
  const addToCart = ({ product, shoeId }) => {
    dispatch(addCarts({ product, shoeId }));
  }
  useEffect(() => {
  ...
  return (
    <div className='container mx-auto min-h-[600px] mt-4'>
      ...
      {(loading && !singleShoeData) ? <LoadingSinglePage /> :
        <div className='flex flex-col gap-10 items-center justify-center text-center md:min-h-[640px] md:flex-row'>
          <div className='basis-1/3 flex-1'>

            <img className='m-auto rounded-lg md:min-w-[400px] lg:min-w-[600px] xl:min-w-[720px] xl:max-w-[720px] xl:min-h-[520px] xl:max-h-[520px] object-cover bg-white' src={singleShoeData?.selectedFile[0]} alt='' />
            <h3 className='text-lg text-justify mb-4'>{singleShoeData?.description}</h3>
          </div>
